    ...
    private function categories(): array
    {
        return [
            ...
            $this->category(
                ...
                self::t('ბიზნეს IT სისტემები და IT ინფრასტრუქტურა', 'Business IT Systems and Infrastructure', 'Бизнес IT-системы и инфраструктура'),
                self::t('ბიზნეს IT სისტემების კატალოგი: POS, კომპიუტერული გარემო, ქსელი, Wi-Fi, სერვერული და სარეზერვო ინფრასტრუქტურა და შესაბამისი ტექნიკური სერვისები. IT მხარდაჭერის სრული მომსახურება წარმოდგენილია ცალკე სპეციალიზებულ გვერდზე.', 'Business IT systems catalog covering POS, workplace computing, networks, Wi-Fi, server and backup infrastructure, with dedicated technical services. Full IT support is presented on its dedicated service page.', 'Каталог бизнес IT-систем: POS, рабочие компьютеры, сети, Wi-Fi, серверная и резервная инфраструктура и профильные технические услуги. Полная IT-поддержка представлена на отдельной странице услуги.'),
                [
                    self::t('ბიზნეს IT სისტემები', 'business IT systems', 'бизнес IT-системы'),
                    self::t('IT ინფრასტრუქტურა', 'IT infrastructure', 'IT-инфраструктура'),
                    self::t('POS და ქსელური სისტემები', 'POS and network systems', 'POS и сетевые системы'),
                ],
            ),
            $this->category(
                'network-infrastructure',
        ...
        $services = [
            $this->service('operating-system-installation', 'computer-services', 'desktop_windows', 'it-support',
                ...
                [
                    ...
                    $this->faq('precheck', self::t('რა მოწმდება ინსტალაციის დაწყებამდე?', 'What is checked before installation?', 'Что проверяется перед установкой?'), self::t('ვადგენთ მოწყობილობის ტექნიკურ მდგომარეობას, დისკზე თავისუფალ ადგილს, საჭირო დრაივერებს, ლიცენზიის მდგომარეობასა და გადასატანი ფაილების მოცულობას. მონაცემებთან დაკავშირებული მოქმედებები წინასწარ თანხმდება მომხმარებელთან.', 'We check the device condition, available storage, required drivers, license status, and the volume of files to migrate. Every action involving customer data is agreed before work begins.', 'Проверяем состояние устройства, свободное место, необходимые драйверы, статус лицензии и объем переносимых файлов. Все действия с данными заранее согласовываются с клиентом.')),
                    $this->faq('handover', self::t('რა მოწმდება სამუშაოს ჩაბარებისას?', 'What is verified at handover?', 'Что проверяется при сдаче работы?'), self::t('ვამოწმებთ სისტემის ჩატვირთვას, ქსელს, ხმას, ეკრანს, მოწყობილობის დრაივერებს, განახლებებსა და შეთანხმებულ პროგრამებს. მომხმარებელს ვუხსნით შესრულებულ ცვლილებებს და მონაცემების მდებარეობას.', 'We verify startup, networking, audio, display, device drivers, updates, and agreed applications. The customer is shown what changed and where migrated data is stored.', 'Проверяем загрузку, сеть, звук, экран, драйверы, обновления и согласованные программы. Клиенту показываем выполненные изменения и расположение перенесенных данных.')),
                ]),
            $this->service('custom-computer-build', 'computer-services', 'memory', 'it-support',
                ...
                [
                    ...
                    $this->faq('planning', self::t('როგორ იგეგმება სწორი კონფიგურაცია?', 'How is the right configuration planned?', 'Как планируется подходящая конфигурация?'), self::t('ვაზუსტებთ პროგრამებს, სამუშაო დატვირთვას, მონიტორის მოთხოვნებს, საცავის მოცულობას, განახლების პერსპექტივასა და ბიუჯეტს. შემდეგ მოწმდება პროცესორის, დედაპლატის, მეხსიერების, ვიდეო ბარათის, კვებისა და კორპუსის თავსებადობა.', 'We clarify applications, workload, display requirements, storage, upgrade plans, and budget. CPU, motherboard, memory, graphics, power supply, cooling, and case compatibility are then verified together.', 'Уточняем программы, нагрузку, требования к монитору, накопителям, будущему обновлению и бюджету. Затем совместно проверяем совместимость процессора, платы, памяти, видеокарты, питания, охлаждения и корпуса.')),
                    $this->faq('handover', self::t('რას მოიცავს აწყობილი კომპიუტერის ჩაბარება?', 'What is included at handover?', 'Что входит в сдачу собранного компьютера?'), self::t('კაბელები წესრიგდება ისე, რომ არ ზღუდავდეს ჰაერის ნაკადს; მოწმდება ყველა პორტი, მეხსიერება და დისკი; ფიქსირდება დატვირთვისას ტემპერატურა და სტაბილურობა. ოპერაციული სისტემა და პროგრამები ემატება მხოლოდ შეთანხმების მიხედვით.', 'Cables are arranged without restricting airflow; ports, memory, and storage are checked; temperatures and stability are verified under load. The operating system and applications are added only when agreed.', 'Кабели укладываются без ухудшения воздушного потока; проверяются порты, память и накопители; под нагрузкой контролируются температуры и стабильность. Система и программы устанавливаются только по согласованию.')),
                ]),
            $this->service('computer-cleaning-maintenance', 'computer-services', 'cleaning_services', 'it-support',
                ...
                [
                    ...
                    $this->faq('diagnostics', self::t('გაწმენდამდე დიაგნოსტიკა რატომ არის საჭირო?', 'Why is diagnosis needed before cleaning?', 'Зачем нужна диагностика перед чисткой?'), self::t('გადახურება ყოველთვის მხოლოდ მტვერს არ უკავშირდება. წინასწარი შემოწმება გვეხმარება განვასხვაოთ დაბინძურება, ქულერის გაუმართაობა, ჰაერის ნაკადის პრობლემა და პროგრამული დატვირთვა, რათა არასაჭირო სამუშაო არ შესრულდეს.', 'Overheating is not always caused by dust. A preliminary check separates contamination, fan failure, airflow issues, and software load so unnecessary work is avoided.', 'Перегрев не всегда вызван пылью. Предварительная проверка помогает различить загрязнение, неисправность вентилятора, проблему воздушного потока и программную нагрузку, чтобы избежать лишних работ.')),
                    $this->faq('aftercare', self::t('როგორ შევინარჩუნო შედეგი გაწმენდის შემდეგ?', 'How can I preserve the result after cleaning?', 'Как сохранить результат после чистки?'), self::t('მოწყობილობა უნდა იდგეს მყარ ზედაპირზე და სავენტილაციო ღიობები თავისუფალი დარჩეს. პერიოდულად აკონტროლეთ ხმაური და ტემპერატურა; მათი მკვეთრი ზრდისას მოწყობილობა დროულად უნდა შემოწმდეს.', 'Keep the device on a firm surface with ventilation openings unobstructed. Monitor noise and temperature periodically; a sudden increase should prompt a timely inspection.', 'Держите устройство на твердой поверхности и не перекрывайте вентиляцию. Периодически контролируйте шум и температуру; резкий рост — повод своевременно провести проверку.')),
                ]),
            ...
            $this->service('business-it-support', 'business-it', 'support_agent', 'it-support',
                ...
                self::t('IT მხარდაჭერა და ტექნიკური მომსახურება ბიზნესისთვის', 'IT Support and Technical Services for Business', 'IT-поддержка и техническое обслуживание бизнеса'),
                self::t('ერთჯერადი და აბონენტური IT მხარდაჭერა ბიზნესისთვის: Windows და კომპიუტერები, პრინტერები, LAN/Wi-Fi, როუტერები, POS, მომხმარებლები, Microsoft 365, VPN, სერვერები, NAS და სარეზერვო ასლები — დისტანციურად ან ადგილზე, სამუშაოს ტიპის მიხედვით.', 'One-time and managed IT support for businesses: Windows and computers, printers, LAN/Wi-Fi, routers, POS, users, Microsoft 365, VPN, servers, NAS and backups, delivered remotely or on site depending on the task.', 'Разовая и абонентская IT-поддержка бизнеса: Windows и компьютеры, принтеры, LAN/Wi-Fi, роутеры, POS, пользователи, Microsoft 365, VPN, серверы, NAS и резервное копирование — удаленно или с выездом в зависимости от задачи.'),
                self::t('IT მხარდაჭერა და IT მომსახურება ბიზნესისთვის | SafeTech', 'Business IT Support and IT Services | SafeTech', 'IT-поддержка и IT-услуги для бизнеса | SafeTech'),
                self::t('IT მხარდაჭერა და ტექნიკური მომსახურება ბიზნესისთვის — კომპიუტერები, Windows, პრინტერები, ქსელი, Wi-Fi, POS, Microsoft 365, VPN, სერვერები, NAS და Backup. ერთჯერადი ან აბონენტური ფორმატი.', 'IT support and technical services for business: computers, Windows, printers, networks, Wi-Fi, POS, Microsoft 365, VPN, servers, NAS and backup. One-time or managed support.', 'IT-поддержка и техническое обслуживание бизнеса: компьютеры, Windows, принтеры, сеть, Wi-Fi, POS, Microsoft 365, VPN, серверы, NAS и резервное копирование. Разовый или абонентский формат.'),
                ...
                ['IT მხარდაჭერა', 'IT მომსახურება ბიზნესისთვის', 'ტექნიკური მხარდაჭერა', 'აბონენტური IT მომსახურება', 'remote IT support Georgia'],
                [
                    $this->faq('format', self::t('დისტანციური მხარდაჭერა გაქვთ?', 'Do you provide remote support?', 'Вы оказываете удаленную поддержку?'), self::t('დიახ. Windows-ის, პროგრამების, ანგარიშების, VPN-ის და ზოგი ქსელური პრობლემის მოგვარება ხშირად დისტანციურად შეიძლება; აპარატურული, კაბელირების ან ადგილზე შესამოწმებელი პრობლემა ტექნიკოსის ვიზიტს მოითხოვს.', 'Yes. Windows, software, account, VPN and some network issues can often be handled remotely; hardware, cabling and site-specific faults require an on-site visit.', 'Да. Проблемы Windows, программ, учетных записей, VPN и часть сетевых задач часто решаются удаленно; аппаратные, кабельные и объектовые неисправности требуют выезда.')),
                    $this->faq('subscription', self::t('აბონენტური მომსახურება შესაძლებელია?', 'Is managed monthly support available?', 'Доступно абонентское обслуживание?'), self::t('დიახ. მომსახურების გეგმა ფორმირდება მოწყობილობების, მომხმარებლების, სისტემების, პრიორიტეტებისა და შეთანხმებული რეაგირების დროის მიხედვით.', 'Yes. The plan is based on devices, users, systems, priorities, and agreed response times.', 'Да. План формируется по устройствам, пользователям, системам, приоритетам и согласованному времени реакции.')),
                    $this->faq('scope', self::t('რა სისტემებს მოიცავს მხარდაჭერა?', 'What systems can be supported?', 'Какие системы входят в поддержку?'), self::t('სამუშაოს შეთანხმებული მოცულობის მიხედვით მხარდაჭერა შეიძლება მოიცავდეს კომპიუტერებს, Windows-ს, პროგრამებს, პრინტერებს, LAN/Wi-Fi-ს, როუტერებს, POS-ს, Microsoft 365-ს, VPN-ს, სერვერებს, NAS-სა და Backup-ს.', 'Depending on the agreed scope, support can cover computers, Windows, software, printers, LAN/Wi-Fi, routers, POS, Microsoft 365, VPN, servers, NAS and backups.', 'В согласованный объем могут входить компьютеры, Windows, программы, принтеры, LAN/Wi-Fi, роутеры, POS, Microsoft 365, VPN, серверы, NAS и резервное копирование.')),
                ]),
            $this->service('security-camera-installation', 'security-access-automation', 'videocam', 'cctv',
                self::t('უსაფრთხოების კამერების დაყენება და მონტაჟი', 'Security Camera Installation and Setup', 'Установка и монтаж камер видеонаблюдения'),
                self::t('IP და ანალოგური კამერების დაყენება, NVR/DVR-ის გამართვა და დისტანციური წვდომა', 'IP and Analog CCTV Installation, NVR/DVR Setup and Remote Access', 'Установка IP и аналоговых камер, настройка NVR/DVR и удаленного доступа'),
                self::t('ობიექტის შეფასება, კამერების წერტილების დაგეგმვა, კაბელირება, PoE ქსელი, კამერების დაყენება და მონტაჟი, NVR/DVR, ჩანაწერის არქივი, მოძრაობისა და ადამიანის დეტექცია და ტელეფონიდან ნახვა.', 'Site assessment, camera placement, cabling, PoE networking, camera installation, NVR/DVR, recording retention, motion and human detection, and mobile viewing.', 'Оценка объекта, размещение камер, кабельная сеть, PoE, установка и монтаж камер, NVR/DVR, архив, детекция движения и людей, просмотр с телефона.'),
                self::t('კამერების დაყენება და მონტაჟი | SafeTech', 'Security Camera Installation and Setup | SafeTech', 'Установка и монтаж камер видеонаблюдения | SafeTech'),
                self::t('უსაფრთხოების კამერების დაყენება და პროფესიონალური მონტაჟი: IP/PoE, NVR/DVR, 24/7 ჩანაწერი, Full Color, UPS და ტელეფონიდან ნახვა.', 'Professional security camera installation and setup: IP/PoE cameras, NVR/DVR, 24/7 recording, Full Color options, UPS and mobile viewing.', 'Профессиональная установка и монтаж камер: IP/PoE, NVR/DVR, запись 24/7, Full Color, ИБП и просмотр с телефона.'),
                ...
                ['კამერების დაყენება', 'კამერების მონტაჟი', 'CCTV installation Georgia', 'NVR DVR configuration'],
                [
            ...
            $this->service('barrier-gate-installation', 'security-access-automation', 'toll', 'access-control',
                ...
                self::t('შლაგბაუმის მონტაჟი და ავტომატური მართვის სისტემები', 'Barrier Gate Installation and Automated Access Control', 'Монтаж шлагбаума и автоматическое управление доступом'),
                self::t('ავტომატური შლაგბაუმის დაგეგმვა, მონტაჟი და კონფიგურაცია პულტით, GSM-ით, RFID/Access Control-ით ან თავსებადი LPR ნომრის ამოცნობით. უსაფრთხოების სქემა შეიძლება მოიცავდეს Photocell-სა და Loop Detector-ს; შესაძლებლობები დამოკიდებულია შერჩეულ კონტროლერსა და მოდელზე.', 'Planning, installation and configuration of automatic barrier gates with remote control, GSM, RFID/access control or compatible LPR plate recognition. Safety can include photocells and loop detectors; available functions depend on the selected controller and model.', 'Проектирование, монтаж и настройка автоматических шлагбаумов с пультом, GSM, RFID/контролем доступа или совместимым LPR-распознаванием номеров. Безопасность может включать фотоэлементы и Loop Detector; функции зависят от выбранного контроллера и модели.'),
                self::t('შლაგბაუმის მონტაჟი — LPR, GSM და ავტომატური მართვა | SafeTech', 'Barrier Gate Installation — LPR, GSM and Automated Access | SafeTech', 'Монтаж шлагбаума — LPR, GSM и автоматический доступ | SafeTech'),
                self::t('შლაგბაუმის მონტაჟი და გამართვა: პულტი, GSM, RFID/Access Control, თავსებადი LPR კამერა, Photocell, Loop Detector, ავტომატური დახურვა და უსაფრთხოების ტესტირება.', 'Barrier installation and setup with remote, GSM, RFID/access control, compatible LPR camera, photocell, loop detector, automatic closing and safety testing.', 'Монтаж и настройка шлагбаума: пульт, GSM, RFID/контроль доступа, совместимая LPR-камера, фотоэлементы, Loop Detector, автозакрытие и проверка безопасности.'),
                ...
                ['შლაგბაუმის მონტაჟი', 'LPR შლაგბაუმი', 'GSM შლაგბაუმი', 'Loop Detector', 'Photocell', 'access control barrier'],
                [
                    ...
                    $this->faq('price', self::t('რა ღირს შლაგბაუმის მონტაჟი?', 'How much does barrier gate installation cost?', 'Сколько стоит монтаж шлагбаума?'), self::t('საბოლოო ფასი დამოკიდებულია გასასვლელის სიგანეზე, შლაგბაუმის მოდელზე, ფუნდამენტსა და კვებაზე, უსაფრთხოების სენსორებზე და მართვის მეთოდზე. ზუსტი შეთავაზება მზადდება ობიექტის მოთხოვნების დაზუსტების შემდეგ.', 'Final cost depends on lane width, barrier model, foundation and power, safety sensors, and the control method. An accurate quote is prepared after the site requirements are confirmed.', 'Итоговая стоимость зависит от ширины проезда, модели шлагбаума, основания и питания, датчиков безопасности и способа управления. Точное предложение формируется после уточнения требований объекта.')),
                    $this->faq('lpr', self::t('როგორ მუშაობს ნომრის ამომცნობი LPR კამერა?', 'How does an LPR plate-recognition camera work?', 'Как работает LPR-камера распознавания номеров?'), self::t('თავსებადი LPR კამერა კითხულობს სანომრე ნიშნებს და კონტროლერის სცენარის მიხედვით დაშვებულ ნომერზე შეიძლება გაიცეს გახსნის სიგნალი. შედეგი დამოკიდებულია კამერის მოდელზე, კუთხეზე, განათებასა და კონფიგურაციაზე.', 'A compatible LPR camera reads license plates and can trigger opening for authorized plates according to the controller logic. Results depend on the camera model, angle, lighting, and configuration.', 'Совместимая LPR-камера считывает номера и по логике контроллера может подавать сигнал открытия для разрешенных номеров. Результат зависит от модели камеры, угла, освещения и настройки.')),
                    $this->faq('control', self::t('შესაძლებელია შლაგბაუმის ტელეფონით გაღება?', 'Can the barrier be opened by phone?', 'Можно открывать шлагбаум с телефона?'), self::t('დიახ, თუ არჩეული სისტემა მხარს უჭერს GSM კონტროლერს, აპლიკაციას ან შესაბამის Access Control ინტეგრაციას.', 'Yes, when the selected system supports a GSM controller, app, or suitable access-control integration.', 'Да, если выбранная система поддерживает GSM-контроллер, приложение или соответствующую интеграцию контроля доступа.')),
                    $this->faq('loop-photocell', self::t('რა განსხვავებაა Loop Detector-სა და Photocell-ს შორის?', 'What is the difference between a loop detector and a photocell?', 'В чем разница между Loop Detector и фотоэлементом?'), self::t('Loop Detector გზის საფარში მოთავსებული ინდუქციური მარყუჟით ავტომობილს აფიქსირებს, Photocell კი სხივის გადაკვეთას აკონტროლებს. კონკრეტულ ობიექტზე შეიძლება გამოიყენებოდეს ერთი ან ორივე უსაფრთხოების სქემის მიხედვით.', 'A loop detector senses a vehicle through an inductive loop in the roadway, while a photocell monitors interruption of an optical beam. One or both may be used depending on the safety design.', 'Loop Detector определяет автомобиль индукционной петлей в покрытии, а фотоэлемент контролирует пересечение светового луча. В зависимости от схемы безопасности может применяться один или оба элемента.')),
                    $this->faq('dual-control', self::t('შეიძლება პულტით და GSM-ით ერთდროულად მართვა?', 'Can remote control and GSM be used together?', 'Можно одновременно использовать пульт и GSM?'), self::t('ხშირად შესაძლებელია, თუ კონტროლერს აქვს შესაბამისი შესასვლელები და არჩეული მოდულები თავსებადია. საბოლოო სქემა კონკრეტული კონტროლერის მიხედვით მოწმდება.', 'Often yes, if the controller has the required inputs and the selected modules are compatible. The final wiring and logic must be verified for the specific controller.', 'Часто да, если у контроллера есть нужные входы и выбранные модули совместимы. Итоговая схема проверяется по конкретному контроллеру.')),
                    $this->faq('upgrade', self::t('შესაძლებელია უკვე დამონტაჟებული შლაგბაუმის განახლება?', 'Can an existing barrier gate be upgraded?', 'Можно модернизировать уже установленный шлагбаум?'), self::t('შესაძლებელია დიაგნოსტიკა და, თავსებადობის შემთხვევაში, მართვის მოდულის, უსაფრთხოების სენსორების, GSM-ის, LPR-ის ან Access Control ინტეგრაციის დამატება. წინასწარ მოწმდება არსებული კონტროლერი და მექანიკა.', 'We can diagnose an existing system and, where compatible, add control modules, safety sensors, GSM, LPR, or access-control integration. The existing controller and mechanics are checked first.', 'Возможна диагностика и, при совместимости, добавление модулей управления, датчиков безопасности, GSM, LPR или интеграции контроля доступа. Сначала проверяются существующий контроллер и механика.')),
                ]),
        ];
